...
(function() {
  ...
  describe('Aggregate', function() {
    ...
    it('first-level, second-level mixed', function() {
      var dockerfile, list, runCommand;
      list = [new Run('echo 123'), new Package('make'), new Run('echo 456'), new Package('bc')];
      functions.aggregate(list, 0);
      should(list.length).be.equal(1);
      dockerfile = [];
      list[0].applyTo({}, dockerfile);
      runCommand = "RUN echo 123 \\\n" + " && apt-get -y install make \\\n" + " && echo 456 \\\n" + " && apt-get -y install bc";
      return should(dockerfile[0]).be.equal(runCommand);
    });
    ...
    it('third, second, first', function() {
      ...
      dockerfile = [];
      list[0].applyTo({}, dockerfile);
      runCommand = "RUN apt-get -y install a make \\\n" + " && echo 123";
      return should(dockerfile[0]).be.equal(runCommand);
    });
    ...
    return it('all levels mixed', function() {
      ...
      dockerfile = [];
      list[0].applyTo({}, dockerfile);
      runCommand = 'RUN apt-get -y install bc make nano \\\n' + ' && echo 123 \\\n' + ' && apt-get -y install libvirt \\\n' + ' && echo 456';
      return should(dockerfile[0]).be.equal(runCommand);
    });
  });
